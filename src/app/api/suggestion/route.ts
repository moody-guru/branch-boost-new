...
export async function POST(request: Request) {
  try {
    ...
    // 'gemini-flash-latest' automatically picks the most stable version for your key
    const model = genAI.getGenerativeModel({ model: "gemini-flash-latest" });

    // Updated Prompt for Broader Audience
    const prompt = `
      You are an expert productivity mentor. 
      A user wants to accomplish the following task: "${taskName}".

      Your instructions are:
      1. Provide a concise, actionable, step-by-step guide to accomplishing this task.
      2. Suggest one useful tool, website, or technique that makes this task easier.
      3. Keep the total response under 150 words.
