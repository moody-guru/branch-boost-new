...
export default function DashboardPage() {
  ...
  const handleDeleteTask = async (taskId: number) => {
    ...
    // FIX: Convert taskId to a string to match the reducer's expected type
    else dispatch({ type: ActionKinds.DELETE_TASK, payload: String(taskId) });
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white p-6 rounded-lg shadow-md h-full transition-all duration-300 hover:shadow-xl">
              ...
              {errorSuggestion && (
                <p className="text-red-500">{errorSuggestion}</p>
              )}
              {suggestion && (
              ...
              {!isLoadingSuggestion && !suggestion && !errorSuggestion && (
                <p className="text-slate-500">
                  Add a new task to get personalized steps and suggestions from
                  your AI mentor!
                </p>
              )}
            </div>
